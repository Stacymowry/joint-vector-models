//Shared species w/ phylogenetic info, pathogen
...
model{
    ...
    for (n in 1:5097)
    {
      for (j in 1:6)
      {
        x[n,j] =  b_0[j] + b_1[j] * dtw_s[n] + b_2[j] * tmax_s[n] 
        + b_3[j] * phdi_s[n] + b_4[j] * ndvi_s[n] + b_5[j] * prcp_s[n]
        + b_6[j] * urban_s[n] + b_7[j] * crop_s[n] + b_9[j] * forest_s[n]
        + b_10[j] * wetland_s[n];
