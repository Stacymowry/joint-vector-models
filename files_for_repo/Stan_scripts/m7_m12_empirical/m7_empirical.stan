// Independent species model, vector only
...
model{
   ...
   // estimate betas in vector model
    for ( i in 1:1492 ) 
    {
        p[i] = b_0[sp[i]] + b_1[sp[i]] * dtw[i] + b_2[sp[i]] * tmax[i] 
              + b_3[sp[i]] * phdi[i] + b_4[sp[i]] * ndvi[i] + b_5[sp[i]] * prcp[i] 
              + b_6[sp[i]] * urban[i] + b_7[sp[i]] * crop[i] + b_9[sp[i]] * forest[i]
              + b_10[sp[i]] * wetland[i];
        p[i] = inv_logit(p[i]);
    }
    y ~ bernoulli( p );
    ...
    for (n in 1:5097)
    {
      for (j in 1:6)
      {
        x[n,j] = b_0[j] + b_1[j] * dtw_s[n] + b_2[j] * tmax_s[n] + b_3[j] * phdi_s[n] 
                + b_4[j] * ndvi_s[n] + b_5[j] * prcp_s[n] + b_6[j] * urban_s[n] 
                + b_7[j] * crop_s[n] + b_9[j] * forest_s[n] + b_10[j] * wetland_s[n];
