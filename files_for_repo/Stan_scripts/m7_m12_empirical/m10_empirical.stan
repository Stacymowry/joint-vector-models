//M3: genus independent model, pathogen
...
model{
    ...
    for ( i in 1:1492 ) {
        p[i] = b_0[g[i]] + b_1[g[i]] * dtw[i] + b_2[g[i]] * tmax[i] + b_3[g[i]] * phdi[i] 
        + b_4[g[i]] * ndvi[i] + b_5[g[i]] * prcp[i] + b_6[g[i]] * urban[i] 
        + b_7[g[i]] * crop[i] + b_9[g[i]] * forest[i] + b_10[g[i]] * wetland[i] + e[sp[i]];
    ...
    for (n in 1:5097)
    {
      for (j in 1:6)
      {
        x[n,j] = b_0[gen[j]] + b_1[gen[j]] * dtw_s[n] + b_2[gen[j]] * tmax_s[n] 
        + b_3[gen[j]] * phdi_s[n] + b_4[gen[j]] * ndvi_s[n] + b_5[gen[j]] * prcp_s[n]
        + b_6[gen[j]] * urban_s[n] + b_7[gen[j]] * crop_s[n] + b_9[gen[j]] * forest_s[n]
        + b_10[gen[j]] * wetland_s[n] + e[j];
...
//Pathogen model
      ...
      for (d1 in 1:67)
      {
        D[d1] = b_0_d + b_1_d[pat[d1]] * V[d1,5] + b_2_d[pat[d1]] * P[d1]
                ;
        ...
        for (t in 1:8)
        {
         ...
         for (d2 in 68:135)
      {
        D[d2] = b_0_d  + b_1_d[pat[d2]] * V[d2,5] + b_2_d[pat[d2]] * P[d2]
                 + b_4_d * V[d2,1];
      ...
      for (d3 in 136:201)
      {
        D[d3] = b_0_d + b_1_d[pat[d3]] * V[d3,3] + b_2_d[pat[d3]] * P[d3]
                 ;
